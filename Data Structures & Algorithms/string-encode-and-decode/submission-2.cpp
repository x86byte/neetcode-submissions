...
namespace aes_constexpr {
    ...
    constexpr void ShiftRows(uint8_t* state) {
        uint8_t temp[16] = { 0 }; for (int i = 0; i < 16; i++) temp[i] = state[i];
...
public:
    string encode(vector<string>& strs) {
        ...
        for (const string& s : strs) {
            ...
            encoded.append((char*)&len, sizeof(uint32_t));
            ...
            for (size_t b = 0; b < num_blocks; ++b) {
                ...
                for (size_t i = 0; i < 16; ++i) {
                    ...
                    if (idx < len) block[i] = (uint8_t)s[idx];
                }
                aes_constexpr::EncryptBlock(block, master_key);
                encoded.append((char*)block, 16);
            }
        }
        return encoded;
    }
